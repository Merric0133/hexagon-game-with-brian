...
class CellEditor:
    ...
    def draw(self, surface, game_time):
        # Distinct editor background — dark blue-grey with a subtle dot grid
        surface.fill((8, 10, 18))
        # Subtle dot grid to signal "UI space, not game world"
        dot_spacing = 32
        dot_color = (20, 22, 36)
        for gx in range(0, self.sw, dot_spacing):
            for gy in range(0, self.sh, dot_spacing):
                pygame.draw.circle(surface, dot_color, (gx, gy), 1)
        ...
        # Zoom indicator
        zoom_val = int(self.zoom * 10) / 10.0
        zoom_txt = self.font_small.render("Zoom: " + str(zoom_val) + "x", True, (80, 120, 140))
        ...
        for row in GRID_ROWS:
            for col in GRID_COLS:
                ...
                if coord in self.layout:
                    ...
                    # Empty hex outline — muted, clearly non-interactive
                    alpha = 18 if coord != self.hover_hex else 55
                    s = pygame.Surface((scaled_hex_r*2+4, scaled_hex_r*2+4), pygame.SRCALPHA)
                    draw_hex(s, (40, 55, 80, alpha), (scaled_hex_r+2, scaled_hex_r+2), scaled_hex_r, width=1)
