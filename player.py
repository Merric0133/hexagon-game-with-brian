...
from constants import (
    ...
    COLOR_ACCENT,
    CellType, CELL_COLORS, CELL_ICONS, CELL_STAT_BONUS,
)
...
class BodyCell:
    ...
    def snap(self, core_x: float, core_y: float):
        ox, oy = self.rest_offset()
        self.px = core_x + ox
        self.py = core_y + oy
        self.vx = 0.0
        self.vy = 0.0

    def update_physics(self, dt: float, core_x: float, core_y: float, cursor_dir: tuple[float, float]):
        ox, oy = self.rest_offset()
        rx, ry = core_x + ox, core_y + oy
        fx = SPRING_K * (rx - self.px) - SPRING_DAMP * self.vx
        fy = SPRING_K * (ry - self.py) - SPRING_DAMP * self.vy
        if self.cell_type == CellType.MOVE:
            cdx, cdy = cursor_dir
            if cdx * cdx + cdy * cdy > 0.01:
                fx += MOVE_FORCE * (rx + cdx * MOVE_LEAD - self.px)
                fy += MOVE_FORCE * (ry + cdy * MOVE_LEAD - self.py)
        self.vx += fx * dt
        self.vy += fy * dt
        self.px += self.vx * dt
        self.py += self.vy * dt
...
class Player:
    ...
    def __init__(self, body_layout: list[dict]):
        ...
        for item in body_layout:
            ...
            if ct != CellType.EMPTY:
                self.cells[(q, r)] = BodyCell(q=q, r=r, cell_type=ct)
    ...
    def update(self, input_handler, dt: float):
        ...
        for cell in self.cells.values():
            cell.update_physics(dt, self.x, self.y, cursor_dir)
    ...
    def _draw_cells(self, surface, camera):
        ...
        for cell in self.cells.values():
            ...
            pts = hex_vertices(cx, cy, HEX_RADIUS)
