...
class Player(Creature):
    ...
    def trigger_keybind_ability(self, key):
        ...
        if key == 'q':  # Dash - quick adjustment for positioning
            self.ability_cooldowns['q'] = self.ability_durations['q']
            angle = self.body.angle
            # Reduced force for quick adjustments, not flying across the map
            dash_force = 3500 * self.body.mass
            # Apply impulse for instant speed, then let physics handle the glide
            ...
            # Reduce damping temporarily for slight glide effect
            self.dash_glide_timer = 0.3  # Glide for 0.3 seconds
            return 'dash'
