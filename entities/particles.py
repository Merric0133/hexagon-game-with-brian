...
class ParticleSystem:
    def __init__(self):
        self.particles = []
        self.projectiles = []  # For attack projectiles
    ...
    def update_projectiles(self, dt):
        ...
        self.projectiles = [p for p in self.projectiles if p["life"] > 0 and not p["hit"]]
    
    def draw_projectiles(self, surface, camera):
        """Draw all projectiles."""
        for proj in self.projectiles:
            sp = camera.world_to_screen(*proj["pos"])
            alpha = int(255 * (proj["life"] / proj["max_life"]))
            r = max(2, int(proj["radius"] * camera.zoom))
            
            # Glow
            draw_glow_circle(surface, proj["glow_color"], sp, r, alpha=int(alpha*0.6), layers=2)
            
            # Core
            c = (*proj["color"][:3], alpha)
            s = pygame.Surface((r*2+1, r*2+1), pygame.SRCALPHA)
            pygame.draw.circle(s, c, (r, r), r)
            surface.blit(s, (sp[0]-r, sp[1]-r))
            
            # Trail
            if proj["vel"][0] != 0 or proj["vel"][1] != 0:
                trail_len = int(math.hypot(proj["vel"][0], proj["vel"][1]) * 0.05)
                if trail_len > 0:
                    trail_x = sp[0] - proj["vel"][0] * 0.02
                    trail_y = sp[1] - proj["vel"][1] * 0.02
                    pygame.draw.line(surface, (*proj["color"][:3], alpha//2), 
                                   (sp[0], sp[1]), (trail_x, trail_y), max(1, r//2))
